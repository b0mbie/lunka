...
//! Data structures like `Box` will not free used memory if they are not dropped
//! in code.
//! However, userdata is properly managed by Lua, and will be collected if an
//! error occurs. Rust code can also use finalizers to run drop glue if the type
//! isn't simply [`Copy`].
//! 
//! ## Data structures can be put into invalid states
//! If a Lua error is raised during the initialization process of a structure,
//! then that structure can be left in an invalid state, which can later be
//! observed externally.
//! 
